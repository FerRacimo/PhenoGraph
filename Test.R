...
for(simidx in seq(1,numsim)){
  ...
  test <- mclapply(1:numproc, function(runnum) {
    ...
    print(tracefile)
    PhenoGraph(tracefile,leaves_counts,neut_leaves_counts,effects,runmode,NaN,numsteps,numsample,innerfreqs_proposize,alpha_prior_stdev,alpha_proposize,ssfactor,qbfactor)
  })
}
